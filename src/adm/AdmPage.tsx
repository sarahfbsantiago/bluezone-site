...
import { CAMPANHAS, CLIENTES, MARCA, PUBLICAR, Scaffold } from './areas/Scaffold'
...
export function AdmPage() {
  ...
  const screen = !canSee(area, role)
    ...
    : area === 'hoje' ? <HojePage posts={posts} role={role} onDrafts={() => openBlueNews({ status: 'draft' })} onNew={() => openBlueNews({ openNew: true })} onOpen={(post) => openBlueNews({ openPost: post })} />
    : area === 'publicar' ? <Scaffold {...PUBLICAR} />
    : area === 'bluenews' ? <BlueNewsArea key={bnInit.nonce} posts={posts} reload={reload} tab={bnTab} onTab={setBnTab} initialSearch={bnInit.search} initialStatus={bnInit.status} openNew={bnInit.openNew} openPost={bnInit.openPost} />
    : area === 'campanhas' ? <Scaffold {...CAMPANHAS} />
    : area === 'clientes' ? <Scaffold {...CLIENTES} />
    : area === 'marca' ? <Scaffold {...MARCA} />
    : <EquipePage email={user.email ?? ''} role={role} demo={demo} />
