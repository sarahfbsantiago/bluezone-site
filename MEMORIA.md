...
- [ ] Resposta automática ao lead ainda NÃO entrega: `testarResposta` diz "enviada" mas nada chega nem fica em Enviados no contato@ (testado com noReply e com envio direto de contato@). Próximo ajuste: colar o Resposta novo (sem noReply), rodar `testarResposta`, ler o Registro de execução e conferir permissão de Gmail; se persistir, testar `MailApp.sendEmail` com `name` em vez de `GmailApp`, e verificar regras de spam/quarentena do Workspace.
- [ ] Verificar relato da cliente: "mando oi em um formulário e chega mensagem de todos" (suspeita de formulários conectados). Observação: no mesmo horário eu disparei 5 testes (um por origem), que geraram 5 avisos no contato@; provável que tenha sido isso. Reproduzir enviando só pelo site e conferindo se chega 1 aviso e 1 linha na planilha.
