...
- **Formulário → Google Apps Script** (conta dmarketingevendas@gmail.com). URL do Web App no `.env` (`VITE_CONTACT_ENDPOINT`). Grava na aba `contatos` da planilha da conta antiga (ID fora do repositório) e envia e-mail. Script com proteção contra fórmulas, injeção no e-mail e limite de envios (30/h, 1 a cada 90 s por e-mail). Validado de ponta a ponta em 11/09/2026.
