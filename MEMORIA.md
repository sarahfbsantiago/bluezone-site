...
- **Apps Script em três arquivos** (`apps-script/Config.gs` = destino, limites e tabela ORIGENS por caminho; `Main.gs` = doPost/doGet/e-mail; `Util.gs` = segurança e planilha), um projeto e uma URL só; passo a passo em `apps-script/PASSO-A-PASSO.txt` (cópia no Desktop da cliente). Ao colar, usar só aspas ASCII: aspas do chat viram tipográficas e quebram o editor.
