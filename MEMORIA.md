...
- 2026-09-13: abaixo do carrossel de clientes, dois iPhones 3D lado a lado (peça `Phone` do Blueprint reaproveitada, brilho azul via `--bp-glow-soft` em `.clients-phones`), giro preso ao scroll (vai e volta). Telas: `public/midia/clientes/wpp-viviane.webp` e `wpp-cliente.webp` (recortadas dos PNGs; fonte em ~/Desktop). Config em `pages.testimonials.phones`. Cartão "Cliente Bluezone" saiu do carrossel (6 cartões). Frase "Porque estratégia não é um documento…" removida.
